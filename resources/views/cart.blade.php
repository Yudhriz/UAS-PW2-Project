                                    ...
                                    @foreach ($cart as $c)
                                        <tr class="table_row">
                                            <td class="column-1">
                                                <form action="{{ route('cart.delete', $c->id) }}" method="POST"
                                                    ...
                                                    <button type="button" class="delete-btn" data-id="{{ $c->id }}">
                                                        <i class="fa fa-trash delete-icon" style="font-size: 18px;"></i>
                                                    </button>
                                                </form>
                                            </td>
                                            <form action="{{ route('cart.update') }}" method="POST">
                                                ...
                                                <td class="column-3">{{ $c->produk->nama }}</td>
                                                <td class="column-4">Rp {{ number_format($c->produk->harga, 0, ',', '.') }}
                                                </td>
                                                <td class="column-5">
                    ...
                    btn.addEventListener('click', function(event) {
                        ...
                        Swal.fire({
                            ...
                            if (result.isConfirmed) {
                                // Redirect to delete route with item ID
                                window.location.href = "{{ route('cart.delete', '') }}/" +
                                    itemId;
                            }
                        });
                    });
